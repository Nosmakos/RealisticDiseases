...
/**
 * Supports 1.13 materials and 1.12 and below at the same time.
 * Includes useful methods to assure full compatibility.
 */
public enum XMaterial {
    ...
    XMaterial(String legacy, int data) {
        this.legacy = legacy;
        this.data = data;
    }

    /**
     * Checks if the the server is running on 1.13 or 1.12 and below.
     *
     * @return true if 1.13 and false if 1.12 and below.
     */
    public static boolean isNewVersion() {
        if (version == 0) return false;
        if (version == 1) return true;

        if (getVersion() != MinecraftVersion.VERSION_1_13) {
            version = 0;
            return false;
        }
    ...
    /**
     * Gets the version the server is running on using materials.
     *
     * @return {@link MinecraftVersion} returns {@link MinecraftVersion#UNKNOWN} if server is running on a version below 1.8
     */
    public static MinecraftVersion getVersion() {
        if (Material.matchMaterial("KELP") != null) return MinecraftVersion.VERSION_1_13;
        if (Material.matchMaterial("ELYTRA") != null) return MinecraftVersion.VERSION_1_9;
        if (Material.matchMaterial("PRISMARINE") != null) return MinecraftVersion.VERSION_1_8;

        return MinecraftVersion.UNKNOWN;
    }

    /**
     * Requests a material from a cached list which saved a list of materials
     * that was used at least once in the server.
     *
     * @param name material's name.
     * @param data {@link ItemStack} durability.
     * @return {@link XMaterial} and null of nothing found.
     */
    public static XMaterial requestXMaterial(String name, byte data) {
        if (isNewVersion()) return valueOf(name);

        if (cachedSearch.containsKey(name.toUpperCase() + "," + data))
            return cachedSearch.get(name.toUpperCase() + "," + data);

        for (XMaterial mat : XMaterial.values()) {
            if (name.toUpperCase().equals(mat.legacy) && ((byte) mat.data) == data) {
                cachedSearch.put(mat.legacy + "," + data, mat);
                return mat;
            }
        }
        return null;
    ...
    public ItemStack buildX(String name, int amount) {
        ItemStack item = isNewVersion() ? new ItemStack(this.pM()) : new ItemStack(this.pM(), amount, (byte) data);
    ...
    @SuppressWarnings("deprecation")
    public ItemStack buildX(String name, List<String> list, int amount) {
        ItemStack item = isNewVersion() ? new ItemStack(this.pM()) : new ItemStack(this.pM(), amount, (byte) data);
        ItemMeta meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES);
        meta.setDisplayName(name);
    ...
    /**
     * Same method as {@link Material#matchMaterial(String)} except it's for XMaterials.
     *
     * @param name materials's name.
     * @return {@link XMaterial} and null if nothing found.
     */
    public static XMaterial matchXMaterial(String name) {
        if (name == null || name.equals(""))
            throw new NullPointerException("Name cannot be null or empty");

        String filtered = format(name);
        XMaterial newMat = valueOf(filtered);

        if (newMat != null) return newMat;
        for (XMaterial oldMat : values())
            if (oldMat.legacy.equals(filtered)) return oldMat;

        return null;
    }

    /**
     * Gets the XMaterial from the material's name and {@link ItemStack} durability.
     *
     * @param key material's name and optional {@link ItemStack} durability. NAME:DURABILITY
     * @return {@link XMaterial} and null if nothing found.
     */
    public static XMaterial parseXMaterial(String key) {
        XMaterial material;
        key = format(key);
        try {
            material = XMaterial.valueOf(key);
            ...
            if (split.length == 1)
                ...
                material = requestXMaterial(split[0], Byte.parseByte(split[1]));
        }
     ...
     *
     * @param name       material's name.
     * @param durability {@link ItemStack} durability.
     * @return {@link XMaterial} and null if nothing found.
     */
    public static XMaterial parseXMaterial(String name, byte durability) {
        XMaterial material;
        name = format(name);
        try {
            material = XMaterial.valueOf(name);
        } catch (IllegalArgumentException ignored) {
            if (durability < 1)
                material = requestXMaterial(name, (byte) 0);
            else
                material = requestXMaterial(name, durability);
        }
    ...
    /**
     * Attempts to format an object like material names.
     *
     * @param name the material name.
     * @return formatted and cleaned material name.
     */
    private static String format(Object name) {
        return name.toString().toUpperCase().replaceAll("\\s+", "_").replaceAll("\\W", "");
    }

    /**
     * Gets the legacy material name of this material.
     *
     * @return legacy material name.
     */
    ...
    /**
     * Gets the legacy itemstack durability of this material.
     *
     * @return legacy {@link ItemStack} durability.
     */
    public int getData() {
        return data;
    }
    ...
    /**
     * Gets an {@link ItemStack} instance of this material including it's durability.
     *
     * @return {@link ItemStack} including the durablility.
     */
    @SuppressWarnings("deprecation")
    public ItemStack pI() {
        Material mat = this.pM();
        return isNewVersion() ? new ItemStack(mat) : new ItemStack(mat, 1, (byte) data);
    }

    /**
     * Gets a {@link Material} instance of this material.
     *
     * @return new {@link Material} or old if using 1.12 and below.
     */
    public Material pM() {
        Material newMat = Material.matchMaterial(this.toString());
        Material oldMat = Material.matchMaterial(legacy);

        return newMat != null ? newMat : oldMat;
    }

    /**
     * Checks if the material is similar to an {@link ItemStack}.
     *
     * @param item {@link ItemStack}
     * @return true if the current material is similar to the {@link ItemStack}.
     */
    @SuppressWarnings("deprecation")
    public boolean isSimilar(ItemStack item) {
        if (isNewVersion()) return item.getType() == this.pM();

        XMaterial material = matchXMaterial(item.getType().toString());
        if (isDamageable(material)) return item.getType() == this.pM();

        return item.getType() == this.pM() && (int) item.getDurability() == this.data;
    }

    /**
     * Checks if a material can be damaged.
     *
     * @param material the material that you want to be checked.
     * @return true if the material can be damaged.
     */
    public boolean isDamageable(XMaterial material) {
        String[] damageables = new String[]{"HELMENT", "CHESTPLATE", "LEGGINGS", "BOOTS", "SWORD", "AXE", "PICKAXE", "SHOVEL", "HOE", "ELYTRA", "TURTLE_HELEMET", "TRIDENT", "HORSE_ARMOR", "SHEARS"};
        return Arrays.asList(damageables).contains(material.toString());
    }

    /**
     * A list of versions that might be useful for compatibility checks.
     * Can be used from {@link #getVersion()}
     */
    public enum MinecraftVersion {
        VERSION_1_8, VERSION_1_9, VERSION_1_13,
        /**
         * Used when a server os running on 1.7 and below.
         */
        UNKNOWN
    }
}